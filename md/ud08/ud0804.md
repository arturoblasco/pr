...
???+ example "Revisa con cuidado el siguiente ejemplo"
    ...
    <br /><br />En este caso lo único que habría que hacer es añadir el modificador `abstract` a la clase `Persona`. El resto de la clase permanecería igual y las clases `Alumno` y `Profesor` no tendrían porqué sufrir ninguna modificación.
...
???+ question "Clases abstractas en la API"
    ...
    ```
    <br />De la que heredan clases instanciable como `Vector` o `ArrayList`.
    <br /><br />
    - La clase `AbstractSequentialList`:
    ```java
    public abstract class AbstractSequentialList<E> extends AbstractList<E>
    ```
    <br />Que hereda de `AbstractList` y de la que hereda la clase `LinkedList`.    
...
???+ example "Revisa con cuidado el siguiente ejemplo"
    ...
    <br /><br />Dado que el método `mostrarDatos` no va a ser implementado en la clase `Persona`, será declarado como abstracto y no se incluirá su implementación:
    ```java
